...
const db = new Database('./src/databases/portfolio.db', { verbose: console.log });

// Function to get portfolio data for a specific user
export function getPortfolio(userId: string): { funds: FundData[] } {
	const stmt = db.prepare('SELECT portfolio_data FROM portfolios WHERE user_id = ?');
...
export async function saveExchange(exchangeRate: ExchangeRatePair): Promise<void> {
	console.log('Saving date');
	console.log(exchangeRate);
	const stmt = db.prepare(`UPDATE currencies SET value = ?, date_time = ? WHERE pair = ?`);
